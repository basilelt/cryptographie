...
# French character probabilities
_FREQ_FR = {
    " ": 0.17,
    "\n": 0.01,
    "e": 0.086,
...
def crack_vigenere(filepath: str, max_key_len: int = 120) -> None:
    ...
    print(f"fichier : {input_path}  ({len(cipher_codes)} caractères)\n")

    key_length, scores = find_key_length(cipher_codes, max_key_len)
    print(f"longueur de clé : {key_length}\n")

    print(f"  {'lag':>6}  {'coïncidence':>11}")
    print("  " + "-" * 22)
    for lag in range(max(1, key_length - 3), min(max_key_len, key_length + 3) + 1):
        marker = "  <" if lag == key_length else ""
        print(f"  {lag:>6}  {scores[lag]:>11.6f}{marker}")
    print()
    ...
    print(f"clé : {key_readable}  (hex: {key.hex()})\n")
